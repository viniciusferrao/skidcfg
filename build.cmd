@echo off
rem Build skidset and run its self-check on Windows.
rem
...
rem SKIDSET.EXE with MSCBUILD.BAT, TCBUILD.BAT or WCLBUILD.BAT and install
rem with that.
rem
...
echo Building skidset...
%CC% %CFLAGS% %EXTRA% -I src -o build\skidset.exe ^
    src\drivers.c src\drvblk.c src\drvscan.c src\setup.c src\scrn.c ^
    src\install.c src\util.c src\skidset.c
...
pushd build
rem .\ and not the bare name. Windows can be configured not to search the
rem current directory for an executable, and where it is, "selfcheck.exe" is
rem not recognized as a command and this script stops one line from the end.
.\selfcheck.exe
...
echo.
echo Built build\skidset.exe and build\selfcheck.exe.
echo For a SKIDSET.EXE the game directory can use, run MSCBUILD.BAT.
popd
endlocal
exit /b 0
